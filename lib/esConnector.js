...
ESConnector.prototype.buildNestedQueries = function(body, model, idName, where) {
  ...
  ESConnector.prototype.buildDeepNestedQueries(true, idName, where, body, rootPath, model);
  ...
  if(addedDocTypeToRootPath) {
    ...
    if(!!rootPath && rootPath.bool && rootPath.bool.must_not && rootPath.bool.must_not.length != 0) {
      rootPath.bool.must.push({
        "bool": {
...
ESConnector.prototype.buildDeepNestedQueries = function(root, idName, where, body, path, model) {
  var self = this;
  _.forEach(where, function(value, key) {
    ...
    if (key === 'and' && Array.isArray(value)) {
      ...
      cond.map(function(c) {
        ...
        self.buildDeepNestedQueries(false, idName, c, body, andPath, model);
      });
      ...
      cond.map(function(c) {
        log('ESConnector.prototype.buildDeepNestedQueries', 'mapped', 'body', JSON.stringify(body, null, 0));
        self.buildDeepNestedQueries(false, idName, c, body, orPath, model);
      });
    } else {
      var spec = false;
      var options = null;
