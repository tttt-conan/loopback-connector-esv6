...
var setupIndex = function setupIndex (indexName) {
  ...
  return self.db.indices.exists(params)
      .then(function(exists) {
        log('ESConnector.prototype.setupIndices', 'self.db.indices.exists('+params.index+')', 'response:', exists);
        if (!exists) {
        ...
        log('ESConnector.prototype.setupIndices', 'self.db.indices.exists('+params.index+')', 'failed:', err);
        return Promise.reject(err);
      });
};
